...
class ReseauNeurone:
    ...
    def backward_pass(self, y_train, output):
        ...
        self.m["W2"] = self.beta1 * self.m["W2"] + (1 - self.beta1) * (change_w["W2"])
        self.m["W1"] = self.beta1 * self.m["W1"] + (1 - self.beta1) * (change_w["W1"])

        self.v["W2"] = self.beta2 * self.v["W2"] + (1 - self.beta2) * (change_w["W2"] ** 2)
        self.v["W1"] = self.beta2 * self.v["W1"] + (1 - self.beta2) * (change_w["W1"] ** 2)

        return self.v, change_w, self.m

    def update_weights(self, v: dict, change_w, m: dict):
        m_hat = {}
        m_hat['W2'] = m['W2']/(1 - self.beta1)
        m_hat['W1'] = m['W1']/(1 - self.beta1)

        v_hat = {}
        v_hat['W2'] = v['W2']/(1 - self.beta1)
        v_hat['W1'] = v['W1']/(1 - self.beta1)

        for key, val in v.items():
            self.params[key] -= (self.lr/np.sqrt(val+self.eps)) * m[key]
    ...
    def train(self, train_list, test_list):
        ...
        for i in range(self.epochs):
            for x in train_list:
                ...
                targets[int(values[0])] = 0.99
                output = self.forward_pass(inputs)
                v, change_w, m = self.backward_pass(targets, output)
                self.update_weights(v, change_w, m)
