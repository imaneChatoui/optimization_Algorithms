...
class ReseauNeurone:
    ...
    def backward_pass(self, y_train, output):
        ...
        delta_w = {}
        delta_w['W2'] = -np.sqrt((self.d['W2'] + self.eps) / (self.v['W2'] + self.eps)) * change_w['W2']
        delta_w['W1'] = -np.sqrt((self.d['W1'] + self.eps) / (self.v['W1'] + self.eps)) * change_w['W1']

        self.d["W2"] = 0.9 * self.d["W2"] + 0.1 * (delta_w["W2"] ** 2)
        self.d["W1"] = 0.9 * self.d["W1"] + 0.1 * (delta_w["W1"] ** 2)

        self.v["W2"] = 0.9 * self.v["W2"] + 0.1 * (change_w["W2"] ** 2)
        self.v["W1"] = 0.9 * self.v["W1"] + 0.1 * (change_w["W1"] ** 2)

        return self.v, change_w, self.d

    def update_weights(self, v: dict, change_w, d: dict):
        for key, val in v.items():
            self.params[key] -= (np.sqrt(d[key]+self.eps)/np.sqrt(val+self.eps)) * change_w[key]
    ...
    def train(self, train_list, test_list):
        ...
        for i in range(self.epochs):
            for x in train_list:
                ...
                targets[int(values[0])] = 0.99
                output = self.forward_pass(inputs)
                v, change_w, d = self.backward_pass(targets, output)
                self.update_weights(v, change_w, d)
